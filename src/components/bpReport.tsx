...
export type Reading = { id: string; at: string; device: string | null; name: string | null; idcard: string | null; systolic: number | null; diastolic: number | null; pulse: number | null }
...
export const BP_PRINT_CSS = `@media print {
...
// เนื้อหารายงานความดัน (ใช้ร่วมทั้งหน้าเจ้าหน้าที่และสาธารณะ) — id="bpreport" ไว้ปริ้น/export
// rows = ผลวัดของคน/กลุ่มที่เลือก (เรียงเวลาใดก็ได้ — จัดเรียงภายในเอง)
export function BpReportBody({ person, rows, byName }: { person: string; rows: Reading[]; byName?: boolean }) {
  const sorted = useMemo(() => rows.slice().sort((a, b) => new Date(a.at).getTime() - new Date(b.at).getTime()), [rows])
  const devices = useMemo(() => {
    const c = new Map<string, number>()
    for (const r of sorted) { const d = r.device || 'ไม่ระบุเครื่อง'; c.set(d, (c.get(d) || 0) + 1) }
    return Array.from(c.entries()).sort((a, b) => b[1] - a[1]).map(([d]) => d)
  }, [sorted])
  const byDevice = (dev: string) => sorted.filter((r) => (r.device || 'ไม่ระบุเครื่อง') === dev)
  const latestOf = (dev: string) => { const a = byDevice(dev); return a[a.length - 1] }
  const inheritedNote = !!byName && sorted.some((r) => !(r.name || '').trim())
  ...
  if (sorted.length === 0) return <div className="text-center text-[#8492A6] py-10">ยังไม่มีข้อมูลของ “{person}”</div>
    ...
    <div id="bpreport" className="bg-white border border-[#E7EDF4] rounded-2xl p-6 flex flex-col gap-5">
      <div className="flex items-start justify-between gap-4 border-b border-[#EEF2F8] pb-4 flex-wrap">
        <div>
          ...
          <div className="text-[13px] text-[#5A6B82] mt-1">ผู้วัด: <b className="text-[#233047]">{person}</b> · วัด {sorted.length} ครั้ง · {devices.length} เครื่อง</div>
          <div className="text-[12px] text-[#8492A6] mt-0.5">ล่าสุด: {fmt(sorted[sorted.length - 1].at)}</div>
          {inheritedNote && <div className="text-[11.5px] text-[#7A44C6] mt-1">* ผลของเครื่องที่ไม่ส่งชื่อ (เช่น Yuwell) ถูกจับเป็นผู้วัดคนนี้ โดยยืมชื่อจากการวัดก่อนหน้าภายใน 15 นาที</div>}
        </div>
      ...
      <div className="print-avoid-break">
        ...
        <p className="text-[11px] text-[#8492A6] mt-1.5">ใช้ผลวัด “ล่าสุด” ของแต่ละเครื่อง · ส่วนต่าง: เขียว = ตรงกัน, เหลือง = ใกล้กัน (≤5), แดง = ต่างมาก</p>
      </div>
      ...
      {/* ประวัติ */}
      <div className="print-avoid-break">
        <div className="text-[13px] font-bold text-[#233047] mb-2">ประวัติการวัด ({sorted.length})</div>
        <div className="overflow-x-auto">
