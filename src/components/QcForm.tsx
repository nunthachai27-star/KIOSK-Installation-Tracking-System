...
type Unit = { id: string; serialNo: string; qc: { status: QcStatus; checklist: unknown; staffId: string | null; keyId: string | null } | null; items: UnitItem[] }
type Mark = { result: 'pass' | 'fail'; by: string; at: string }
type Marks = Record<string, Mark | undefined>
type UnitState = { checklist: Marks; status: QcStatus; staffId: string; keyId: string }
...
export function QcForm({
  ...
  const [state, setState] = useState<Record<string, UnitState>>(() => {
    ...
    for (const u of units) m[u.id] = { checklist: initialChecklist(u.qc?.checklist), status: u.qc?.status ?? 'PENDING', staffId: u.qc?.staffId ?? '', keyId: u.qc?.keyId ?? '' }
    return m
  })
  ...
  async function save(unitId: string) {
    ...
    try {
      const res = await fetch(`/api/jobs/${jobId}/units/${unitId}/qc`, {
        ...
        body: JSON.stringify({ status: st.status, staffId: st.staffId || null, checklist, keyId: st.keyId || null }),
      })
