...
export function QueueManager({
  ...
  const field = 'border border-[#D6DFEA] rounded-xl px-3 py-2.5 outline-none focus:border-[#EA580C] focus:ring-2 focus:ring-[#EA580C]/15 transition'
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="ds-card p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <select
            ...
            onChange={(e) => {
              ...
              setJobId(id)
              const d = jobs.find((j) => j.id === id)?.date
              if (d) setWhen(d)
            }}
        ...
        <div className="text-xs text-[#8492A6] mt-2">วันที่จะเติมจากงานให้อัตโนมัติ (กำหนดส่ง/วันสัญญา) — แก้ไขได้</div>
        {err && <div className="text-sm text-[#C13540] mt-2">{err}</div>}
        <button onClick={add} disabled={saving}
          className="ds-hover mt-4 bg-[#EA580C] text-white font-semibold rounded-xl px-5 py-2.5 hover:bg-[#C2410C] disabled:opacity-60">
      ...
      <div className="ds-card overflow-hidden">
        ...
        {items.map((it) => (
          <div key={it.id} className="ds-hover grid grid-cols-[70px_1.2fr_1.3fr_1fr_60px] items-center gap-2 px-5 py-3 border-t border-[#EEF2F8] hover:bg-[#F8FAFD]">
            <div className="font-bold tnum text-[#1C1917]">{it.time}</div>
