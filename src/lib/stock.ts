...
export type LotSummary = { id: string; lotCode: string; received: number; issued: number; remaining: number }
export type ProductSummary = {
  id: string; name: string; group: string; unit: string; lowStockQty: number
  received: number; issued: number; remaining: number; level: StockStatusLevel; lots: LotSummary[]
}
export type GroupSummary = { group: string; received: number; issued: number; remaining: number; products: ProductSummary[] }

export async function getStockSummary() {
  const [products, lots, issuedByLot] = await Promise.all([
  ...
  for (const l of lots) {
    ...
    arr.push({ id: l.id, lotCode: l.lotCode, received, issued, remaining: received - issued })
    lotsByProduct.set(l.productId, arr)
  }

  const groupMap = new Map<string, GroupSummary>()
  let kReceived = 0, kIssued = 0, kRemaining = 0, kLow = 0, kOut = 0
  for (const p of products) {
    ...
    kReceived += received; kIssued += issued; kRemaining += remaining
    if (level === 'LOW') kLow++; if (level === 'OUT') kOut++
    const ps: ProductSummary = { id: p.id, name: p.name, group: p.group, unit: p.unit, lowStockQty: p.lowStockQty, received, issued, remaining, level, lots: plots }
    const g = groupMap.get(p.group) ?? { group: p.group, received: 0, issued: 0, remaining: 0, products: [] }
    g.received += received; g.issued += issued; g.remaining += remaining; g.products.push(ps)
    groupMap.set(p.group, g)
  }

  const groups = [...groupMap.values()].sort((a, b) => b.received - a.received)
  return { kpi: { received: kReceived, issued: kIssued, remaining: kRemaining, low: kLow, out: kOut, products: products.length }, groups }
}
